...
| 16/09/2026 | `P01-K02-manual` (tentativa única) | A implementação em `src/solution.py` estava correta (5/5 testes passam contra `katas/K02/tests`), mas os campos de nível superior do registro bruto não refletem isso: em `trial.json`, o penúltimo poll (23:47:21) já registrou 5/5, porém o `final_check`, rodado no mesmo segundo, capturou 0/5 com o traceback do esqueleto original (`raise NotImplementedError`) — o save final do participante só saiu 11s depois (23:47:32), após o cronômetro já ter fechado o resultado. Causa raiz: `run_trial()` descartava o resultado do poll que já tinha tido sucesso e rodava a suíte de novo para compor o resultado final, abrindo uma janela de corrida entre o cronômetro e o último salvamento do participante. | Trial **mantido como válido**, não invalidado: refazê-lo mediria o participante reescrevendo uma solução que já conhece, o que comprometeria a validade do dado mais do que o bug de captura em si. `trial.json`/`final_junit.xml` foram preservados sem edição (nunca se edita evidência bruta); a leitura correta (concluído, 5/5, ~885s) está documentada em `data/raw/trials/P01-K02-manual/data-quality-note.json`. A causa raiz foi corrigida em `src/collection/trial_collector.py` — quando um poll já teve sucesso, o final_check reaproveita esse resultado em vez de rodar a suíte de novo — para não se repetir nos trials de P02/P03. | Jonathan, com Claude Code |
