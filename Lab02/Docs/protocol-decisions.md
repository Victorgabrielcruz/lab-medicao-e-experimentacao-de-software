...
A versão exata do interpretador e de cada ferramenta ainda precisa ser
travada quando o ambiente for montado (mesma Seção 9) — não porque a escolha
esteja em aberto, mas porque isso só faz sentido registrar no momento em que
o ambiente for de fato instalado nas máquinas dos três integrantes.
...
autocomplete inline em editor, então esse modo não se aplica aqui. Os
detalhes completos (versão, modelo padrão ativo, plano) estão na tabela da
[Seção 6.1 de `methodology.md`](methodology.md#61-tratamento-ia); versão e
modelo serão preenchidos no início da janela de coleta, junto com o resto do
registro de ambiente, porque a CLI é atualizada com frequência.
...
A versão exata do pytest será registrada junto com o restante do ambiente
(Seção 9 de `methodology.md`).
