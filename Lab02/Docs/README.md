# Índice da documentação — Lab02

## Planejamento e processo

| Documento | Conteúdo |
|---|---|
| [`Enunciado.md`](Enunciado.md) | Enunciado original do laboratório |
| [`methodology.md`](methodology.md) | Metodologia completa do experimento |
| [`tasks.md`](tasks.md) | Plano de execução por sprint |
| `execution-guide.md` | Procedimento operacional dos trials — a desenvolver na S01 |
| `protocol-decisions.md` | Decisões congeladas do protocolo — a preencher na S01 |

## Subpastas planejadas

| Pasta | Conteúdo |
|---|---|
| `dataset/` | Esquemas dos dados brutos e processados |
| `templates/` | Modelos de registros e relatório final |
| `validation/` | Procedimentos e evidências de validação |

Documentos indicados como “a desenvolver” serão produzidos pelas tarefas correspondentes de [`tasks.md`](tasks.md).
